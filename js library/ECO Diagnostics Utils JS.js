 ...
 *
 * Vollstndige Library mit allen Funktionen inklusive HTML & CSS Templates:
...
/**
 * Gibt Farbe und Label fr Progress Status zurck
 *
...
/**
 * Gibt HTML Badge  Progress Status zurueck
 *
 * @param {string} progress - Progress Status
...
/**
 * Gibt HTML Badge fr Measurement Type zurck
 *
 * @param {string} measurementType - Measurement Type
 * @returns {string} HTML String
 */
 ...
 * @returns {Object} { icon, color, bgColor, label }
 */
export function getDeviceStatusStyle(hasDevice,
deviceName) {
    let icon, color, bgColor, label;
 ...
 */
export function getActivityStatusStyle(lastActivityTime,
    timeoutMinutes = 5) {
    ...
    const activityMs = typeof lastActivityTime ===
        'string' ?
...
/**
 * Gibt Icon, Farben, CSS-Klasse und i18n-Key fr Alarm State zurck
 *
...
export function getAlarmStateStyle(state) {
    ...
    switch (state) {
        case "critical":
            ...
            i18nKey =
                "custom.diagnostics.alarm-severity.critical";
            break;
        case "major":
            icon = "warning";
            color = "#FF9800";
            ...
            i18nKey =
                "custom.diagnostics.alarm-severity.major";
            break;
        case "minor":
            icon = "info";
            color = "#F9A825";
            ...
            i18nKey =
                "custom.diagnostics.alarm-severity.minor";
            break;
        case "normal":
            icon = "check_circle";
            color = "#4CAF50";
            ...
            i18nKey =
                "custom.diagnostics.alarm-severity.normal";
            break;
        default:
            icon = "help";
            color = "#666";
...
/**
 * Gibt Icon, Farben, CSS-Klasse und i18n-Key fr Progress State zurck
...
export function getProgressStyle(progress) {
    ...
    switch (progress) {
        case "in preparation":
            ...
            i18nKey =
                "custom.diagnostics.state-filter.preparation.title";
            break;
        case "active":
            icon = "play_circle";
            color = "#F9A825";
            ...
            i18nKey =
                "custom.diagnostics.state-filter.active.title";
            break;
        case "finished":
            icon = "check_circle";
            color = "#4CAF50";
            ...
            i18nKey =
                "custom.diagnostics.state-filter.finished.title";
            break;
        case "aborted":
            icon = "cancel";
            color = "#F44336";
            ...
            i18nKey =
                "custom.diagnostics.state-filter.aborted.title";
            break;
        case "disconnected":
            icon = "link_off";
            color = "#757575";
...
/**
 * Gibt HTML Badge mit CSS-Klassen fr State zurck
 *
 * @param {string} stateValue - State Wert
 * @param {string} type - "state" fr Alarm State, "progress" fr Progress State
...
export function getStateBadgeHtml(stateValue, type) {
    ...
    return (
        ...
        .badgeClass + '">' +
        '<mat-icon class="badge-icon">' + style.icon +
        "</mat-icon>" +
...
/**
 * Gibt HTML Badge fr Datum zurck
 *
 * @param {number|string} timestampMs - Unix timestamp in ms
 * @param {string} type - "start" oder "end"
...
export function getDateBadgeHtml(timestampMs, type) {
    ...
    const icon = type === "start" ? "play_circle" :
        "stop_circle";
...
/**
 * Gibt HTML Badge mit Inline-Styles zurck (generisch)
 *
...
/**
 * Initialisiert die Adresssuche fr ein Angular FormGroup
 *
 * VERWENDUNG:
 ...
 * @param {Function} onResultsCallback - Optional: Callback fr Suchergebnisse
 * @returns {Object} API mit searchAddress, onAddressSelected, displayAddressOption
 */
export function initAddressSearch(formGroup, vm,
    onResultsCallback) {
    ...
    function searchAddress() {
        ...
        if (!addressField) {
            console.error(
                '[Address Search] Address field not found in form group'
                );
            return;
        }
        ...
        const q = (typeof qRaw === 'string') ? qRaw.trim() :
            displayAddressOption(qRaw).trim();
    ...
    function onAddressSelected(opt) {
        ...
        const currentPostalCode = formGroup.get(
                'postalCode') ?
            (formGroup.get('postalCode').value || '')
        .trim() : '';
        const currentCity = formGroup.get('city') ?
            (formGroup.get('city').value || '').trim() : '';

        if (!currentPostalCode && opt.postcode && formGroup
            .get('postalCode')) {
            patchData.postalCode = opt.postcode;
        }
        if (!currentCity && opt.city && formGroup.get(
                'city')) {
        ...
        if (formGroup.get('latitude')) formGroup.get(
            'latitude').markAsDirty();
        if (formGroup.get('longitude')) formGroup.get(
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (onResultsCallback &&
                    typeof onResultsCallback ===
                    'function') {
                    onResultsCallback(vm
                    .addressOptions);
                }
            })
            .catch(err => {
                console.error(
                    '[Address Search] Failed:', err);
                vm.addressOptions = [];
            });
    }
    ...
    function setupAutoSearch() {
        ...
        if (!addressField || !addressField.valueChanges) {
            console.warn(
                '[Address Search] Address field does not support valueChanges'
                );
            return;
        }
        ...
        addressField.valueChanges.subscribe(val => {
            ...
            if (typeof val === 'string' && vm
                ._lastSelectedAddressLabel &&
                val === vm._lastSelectedAddressLabel
                ) {
                return;
            }

            const s = (typeof val === 'string') ?
                val.trim() : (displayAddressOption(
                    val) || '').trim();
...
/**
 * Generiert HTML Template fr Adresssuche
 *
...
export function getAddressSearchTemplate(options) {
    ...
    const showPostalCodeCity = opts.showPostalCodeCity !==
        false;
    const showCoordinates = opts.showCoordinates !== false;
    const coordinatesSideBySide = opts
        .coordinatesSideBySide !== false;
    ...
    let template =
        ...
        '          [disabled]="(' + formGroupName +
        '.get(\'address\').value || \'\').length < 5">\n' +
...
/**
 * Gibt das HTML Template fr den Add Project Dialog zurck
 *
...
/**
 * Gibt das CSS fr den Add Project Dialog zurck
 *
 * VERWENDUNG in Custom Action:
...
export function initProjectDialog(widgetContext) {
    ...
    const customDialog = $injector.get(widgetContext
        .servicesMap.get('customDialog'));
    const assetService = $injector.get(widgetContext
        .servicesMap.get('assetService'));
    const entityGroupService = $injector.get(widgetContext
    ...
    const entityRelationService = $injector.get(
        widgetContext.servicesMap.get(
            'entityRelationService'));
    const customerService = $injector.get(widgetContext
        .servicesMap.get('customerService'));

    const ctx = widgetContext;
    const isTenantAdmin = ctx.currentUser.authority ===
        'TENANT_ADMIN';
    const pageLink = ctx.pageLink(1000, 0, null, null,
    null);

    // Config object - wird als Referenz zurckgegeben und spter befllt
    const config = {
    ...
    /**
     * Berechnet die nchste Project ID aus bestehenden Projekten
     */
    ...
    /**
     * ffnet den Add Project Dialog
     */
    function openDialog() {
        customDialog.customDialog(globalThis.htmlTemplate,
            AddProjectDialogController).subscribe();
    }
    ...
    function AddProjectDialogController(instance) {
        ...
        const addressSearch = initAddressSearch(vm
            .addProjectFormGroup, vm);
        vm.searchAddress = addressSearch.searchAddress;
        vm.onAddressSelected = addressSearch
            .onAddressSelected;
        vm.displayAddressOption = addressSearch
        ...
        if (!config.isTenantAdmin && config.customerId) {
            ...
            vm.addProjectFormGroup.get('customer')
                .markAsDirty();
        ...
        vm.onCustomerChange = function() {
            const c = vm.addProjectFormGroup.value
                .customer;
            if (!c) return;
        ...
        vm.save = function() {
            ...
            saveProjectObservable(config.customerId)
                .subscribe(
                    function(Project) {
                        widgetContext.rxjs.forkJoin([
                            saveCustomerToProjectRelation(
                                config
                                .customerId,
                                Project.id),
                            saveAttributes(
        ...
        /**
         * Ldt Customer-Daten und berechnet nchste Project ID
         */
        function loadCustomerData(customerId) {
            ...
            assetService.findByQuery(assetSearchQuery)
                .subscribe(projects => {
                    const nextProjectId =
                        getLastProjectId(projects);

                    attributeService
                        .getEntityAttributes(customerId,
                            'SERVER_SCOPE', [
                                'shortName'])
        ...
        function getProjectsGroup(customerId) {
            return entityGroupService
                .getEntityGroupsByOwnerId(customerId
                    .entityType, customerId.id, 'ASSET')
                .pipe(
                    widgetContext.rxjs.switchMap((
                        entityGroups) => {
                        let ProjectsGroup = entityGroups
                            .find(group => group
                                .name === 'Projects');
                        if (ProjectsGroup) {
         ...
         */
        function saveCustomerToProjectRelation(customerId,
            ProjectId) {
            const relation = {
                from: customerId,
                to: ProjectId,
    ...
    // Return Configuration Object (Referenz, wird von openAddProjectDialog befllt)
    return config;
}

/**
 * Vereinfachte Funktion: ffnet Add Project Dialog direkt
 *
 * VERWENDUNG in Custom Action (einfachste Variante):
 *   utils.openAddProjectDialog(widgetContext);
 *
 * Diese Funktion ist KOMPLETT STANDALONE - kein HTML/CSS in der Custom Action ntig!
 *
 * @param {Object} widgetContext - ThingsBoard Widget Context
 */
export function openAddProjectDialog(widgetContext) {
    ...
    config.services.customerService.getUserCustomers(config
        .pageLink).subscribe(pageData => {
        config.customers = (pageData && pageData
            .data) ? pageData.data : [];

        // Non-Tenant-Admin / genau ein Customer
        if (!config.isTenantAdmin && config
            .customers.length === 1) {
            const c = config.customers[0];
            config.customerId = {
                id: c.id.id,
 ...
 */
export function deleteProject(widgetContext, projectId,
    projectName, callback) {
    const $injector = widgetContext.$scope.$injector;
    const dialogs = $injector.get(widgetContext.servicesMap
        .get('dialogs'));
    const assetService = $injector.get(widgetContext
        .servicesMap.get('assetService'));
    const deviceService = $injector.get(widgetContext
    ...
    function gatherProjectInfo() {
        ...
        findCustomerId().then(function() {
            ...
            assetService.findByQuery(
                measurementsQuery).subscribe(
    ...
    function findCustomerId() {
        return new Promise(function(resolve) {
            ...
            entityRelationService.findByTo(
                projectId, 'Owns').subscribe(
                function(relations) {
                    if (relations && relations
                        .length > 0) {
                        customerId = relations[
                            0].from;
                    }
                    resolve();
                },
    ...
    function findKitsAndShowDialog() {
        ...
        entityGroupService.getEntityGroupsByOwnerId(
            ...
            rxjs.switchMap(function(assetGroups) {
                const kitsGroup = assetGroups.find(
                    function(g) {
                        return g.name ===
                            'Diagnostic Kits';
                    });
            ...
            rxjs.switchMap(function(kitsPage) {
                ...
                const kitChecks = kits.map(function(
                    kit) {
            ...
            function(error) {
                console.error('Error finding kits:',
                    error);
                showConfirmDialog();
            }
        );
    ...
    function showConfirmDialog() {
        ...
        if (measurements.length > 0) {
            parts.push(measurements.length +
                ' measurement' + (measurements.length >
                    1 ? 's' : '') + ' will be deleted');
        }
        if (allDevices.length > 0) {
            parts.push(allDevices.length + ' device' + (
                    allDevices.length > 1 ? 's' : '') +
                ' will be unassigned');
        }
        if (allKits.length > 0) {
            parts.push(allKits.length + ' kit' + (allKits
                    .length > 1 ? 's' : '') +
                ' will be unassigned');
        }
        ...
        dialogs.confirm(title, content, 'Cancel', 'Delete')
    ...
    function performDelete() {
        ...
        unassignKits().then(function() {
            ...
            deleteMeasurementsSequentially(0).then(
                function() {
        ...
        return new Promise(function(resolve) {
            ...
            entityGroupService
                .getEntityGroupsByOwnerId(customerId
                    ...
                    function(error) {
                        console.error(
                            'Error getting unassigned kits group:',
                            error);
                        resolve();
                    }
                );
    ...
    function deleteMeasurementsSequentially(index) {
        return new Promise(function(resolve) {
            ...
            deviceService.findByQuery(devicesQuery)
                .subscribe(
                    function(devices) {
                        if (devices && devices
                            .length > 0 &&
                            ...
                            getOrCreateUnassignedDevicesGroup
                                ().then(function(
                                    unassignedGroupId
                                    ) {
                                    unassignDevices
                                        ...
                                        .then(
                                            function() {
                                                deleteMeasurementAsset
                                                    (
                                                        measurementId)
                                                    .then(
                                                        function() {
                                                            deleteMeasurementsSequentially
                                                                (index +
                                                                    1
                                                                    )
                                                                .then(
                    ...
                    function(error) {
                        console.error(
                            'Error finding devices for measurement:',
                            error);
                        deleteMeasurementAsset(
                            measurementId).then(
                            function() {
                        ...
                        let unassignedGroup =
                            deviceGroups.find(
                                function(group) {
                                    return group
                                        .name ===
                                        'Unassigned Measurement Devices';
                                });

                        if (unassignedGroup) {
                            resolve(unassignedGroup
                                .id.id);
                        } else {
                            const newGroup = {
                                type: 'DEVICE',
                            ...
                            entityGroupService
                                ...
                                .subscribe(
                                    ...
                                    error) {
                                        console
                                            .error(
                                                'Error creating unassigned devices group:',
                                                error
                                                );
                                        resolve(
                                            null);
    ...
    function unassignDevices(devices, unassignedGroupId,
        measurementId) {
        return new Promise(function(resolve) {
            if (!unassignedGroupId || devices
                .length === 0) {
                resolve();
                return;
            }
    ...
    function deleteMeasurementAsset(measurementId) {
        return new Promise(function(resolve) {
            assetService.deleteAsset(measurementId
                .id).subscribe(
                function() {
                    resolve();
                },
